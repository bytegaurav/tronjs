/*
    ...
    website: www.gauravkushwaha.net
*/
...
var Tron = new function () {
    ...
    var rebuildTemplateDOM = function()
    {
        for (var i = 0; i < templateIDs.length; i++) {
            ...
            $(templateDOM).find('[data-templateID]').html(t);
    ...
    var compileTemplate1 = function (item, repeatelements) {
        
        //preparing js string from html
        //fetching template html string 
        var finalhtml = "";
        debugger;
        ...
        {
            repeatertemplate = repeatertemplate.replace(new RegExp("{{", "g"), "'+");
            repeatertemplate = repeatertemplate.replace(new RegExp("}}", "g"), "+'");
            repeatertemplate=repeatertemplate.replace(/(?:\r\n|\r|\n)/g, '');
            obj = item[j];
            htmltemplate = "dom='" + repeatertemplate.trim() + "';";
    ...
    var binding = function () {
        ...
        $(window).on("hashchange", actionChange);
        $(window).on("load", init);
    ...
    var initRoutes=function(){
        routes = new Array();
        $.get(appRoot + "/route.js", function (data) {
    ...
    this.initialise = function (placeholdertag, root, home, viewCaching) {
